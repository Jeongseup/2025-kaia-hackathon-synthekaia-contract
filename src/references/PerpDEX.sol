// ref; https://github.com/devKBit/k-bit/blob/main/src/contracts/PerpDex.sol
...
interface IBisonAIRouter {
    ...
    ) external view returns (uint64 id, int256 answer, uint256 updatedAt);
...
library PerpDexPricesLib {
    function submitAndGetLatestPrice(
        ...
        require(
            priceData.feedHashes.length == 1 &&
                priceData.answers.length == 1 &&
                priceData.timestamps.length == 1 &&
        ...
        if (priceData.oracleType == PerpDexLib.OracleType.BisonAI) {
            bisonAISubmissionProxy.submitSingleWithoutSupersedValidation(
                priceData.feedHashes[0],
                priceData.answers[0],
                priceData.timestamps[0],
                priceData.proofs[0]
            );
            require(
                bisonAISubmissionProxy.lastSubmissionTimes(
                    priceData.feedHashes[0]
                ) >= priceData.timestamps[0],
    ...
    function submitAndGetBisonAIRoundId(
        ...
        require(
            priceData.oracleType == PerpDexLib.OracleType.BisonAI,
            "Wrong oracle type"
        );
        require(
            priceData.feedHashes.length == priceData.answers.length &&
                priceData.feedHashes.length == priceData.timestamps.length &&
        ...
        for (uint256 i = 0; i < priceData.feedHashes.length; i++) {
            require(
                bisonAISubmissionProxy.lastSubmissionTimes(
                    priceData.feedHashes[i]
                ) >= priceData.timestamps[i],
                "Price is not up to date (BisonAI)"
            ); // Should not happen.
            PerpDexLib.TokenType tokenType = PerpDexLib.TokenType(i);
            string memory feedName = getBisonAIFeedName(tokenType);
            (uint64 roundId, int256 answer, ) = bisonAIRouter.latestRoundData(
                feedName
            );
            require(answer > 0, "Price is 0 (BisonAI)");
    ...
    function getPythPrice(
        ...
        IPyth.PythPrice memory pythPriceData = pyth.getPriceNoOlderThan(
            feedHash,
            10
        );
        ...
        if (
            ...
            tokenType == PerpDexLib.TokenType.Shib
        ) {
    ...
    function getPythFeedHashOrder() internal pure returns (bytes32[] memory) {
        bytes32[] memory feedHashes = new bytes32[](
            uint256(type(PerpDexLib.TokenType).max) + 1
        );
        feedHashes[
            0
        ...
        feedHashes[
            1
        ] = 0x452d40e01473f95aa9930911b4392197b3551b37ac92a049e87487b654b4ebbe;
        feedHashes[
            2
    ...
    function checkPythPriceFeedOrder(
        IPyth.PythPriceFeed[] memory priceFeeds
    ) internal pure {
        bytes32[] memory feedHashOrder = getPythFeedHashOrder();
        for (uint256 i = 0; i < priceFeeds.length; i++) {
            require(
                priceFeeds[i].id == feedHashOrder[i],
                "Feed hash is not correct"
            );
        }
    }

    function checkPriceDataOrder(
        PerpDexLib.OraclePrices calldata priceData
    ) internal pure {
        if (priceData.oracleType == PerpDexLib.OracleType.BisonAI) {
            for (uint256 i = 0; i < priceData.feedHashes.length; i++) {
                require(
                    priceData.feedHashes[i] ==
                        keccak256(
                            abi.encodePacked(
                                getBisonAIFeedName(PerpDexLib.TokenType(i))
                            )
                        ),
            ...
            for (uint256 i = 0; i < priceData.feedHashes.length; i++) {
                require(
                    priceData.feedHashes[i] == feedHashOrder[i],
    ...
    function getPreviousPriceAndTime(
        ...
        if (priceData.oracleType == PerpDexLib.OracleType.BisonAI) {
            ...
            for (uint256 i = 0; i < roundIds.length; i++) {
                (, int256 answer, uint256 updatedAt) = bisonAIRouter
                    .getRoundData(
                        getBisonAIFeedName(PerpDexLib.TokenType(i)),
                        roundIds[i]
                    );
            ...
            uint256 feeAmount = pyth.getUpdateFee(priceData.proofs);
            IPyth.PythPriceFeed[] memory priceFeeds = pyth
                .parsePriceFeedUpdates{value: feeAmount}(
                priceData.proofs,
                priceData.feedHashes,
                uint64(priceData.timestamps[0]),
                uint64(priceData.timestamps[0])
            );
            checkPythPriceFeedOrder(priceFeeds);

            uint256[] memory prices = new uint256[](
                priceData.feedHashes.length
            );
            ...
            for (uint256 i = 0; i < priceFeeds.length; i++) {
                ...
                require(
                    price == priceData.answers[i],
                    "Price is not correct (Pyth)"
                );
                prices[i] = uint256(uint64(price));

                require(
                    priceFeeds[i].price.publishTime == priceData.timestamps[i],
    ...
    function safeTransferFromAndCheckBalance(
        address from,
        address to,
        uint256 amount,
        IERC20 usdt
    ...
    function safeTransferAndCheckBalance(
        address to,
        uint256 amount,
        IERC20 usdt
    ) public {
...
library PerpDexAuthLib {
    ...
    function checkUser(
        ...
        bytes32 ethHashedMessage = keccak256(
            abi.encodePacked(
        ...
        if (recoveredAddress != user) {
            bytes32 klayHashedMessage = keccak256(
                abi.encodePacked(
    ...
    function getOpenLimitOrderMsg(
        PerpDexLib.OpenLimitOrderData calldata o,
        uint256 nonce,
    ...
    function getCloseLimitOrderMsg(
        uint256 positionId,
        uint256 nonce,
        address contractAddr
    ) external view returns (string memory) {
...
library PerpDexLib {
    ...
    function checkExecutionForLimitOrder(
        PerpDexLib.Position memory limitOrder,
    ...
    // for liquidation
    function calculatePnlAndCloseFee(
        Position memory position,
        uint256 currentPrice,
        int256 fundingFee,
        IFee feeContract
    ) public view returns (int256, uint256) {
        uint256 finalValueUsdt = (position.size * currentPrice) /
        ...
        if (pnl > 0) {
            ...
            fee =
                ((position.size + profit) * feeContract.getTotalFeePercent()) /
                feeContract.getFeeDenominator();
        } else {
            if (int256(position.margin) < fundingFee) {
                return (0, 0); // margin < funding fee means liquidation, so pnl and closeFee is 0.
            }

            uint256 loss = Math.min(
                uint256(int256(position.margin) - fundingFee),
                uint256(-pnl)
            );
            pnl = -int256(loss);
            fee =
                ((position.size - loss) * feeContract.getTotalFeePercent()) /
                feeContract.getFeeDenominator();
        }
    ...
    function checkAndLiquidatePosition(
        ...
        if (marginAfterFundingFee - int256(closeFee) <= 0) {
            ...
            if (pnl > 0) {
                ...
                int256 deficitCloseFee = int256(closeFee) -
                    marginAfterFundingFee;
                if (pnl > deficitCloseFee) {
                    // Profit can pay all the deficit amount
                    uint256 profitAfterCloseFee = uint256(
                        pnl - deficitCloseFee
                    );
                    ...
                    payAndRecordFee(
                        position,
                        uint256(marginAfterFundingFee + pnl),
                        false,
                        c.feeContract
                    );
                }
        ...
        if (pnl < 0 && marginAfterFundingFee - int256(closeFee) + pnl <= 0) {
            ...
            takeAndRecordLoss(
                position,
                uint256(marginAfterFundingFee) - closeFee,
                c.lp,
                c.usdt
            );
    ...
    function calculateOpenFee(
        uint256 inputMarginAmount,
        uint256 inputLeverage,
        IFee feeContract
    ) public view returns (uint256) {
        uint256 inputSize = inputMarginAmount * inputLeverage;
        uint256 fee = (inputSize * feeContract.getTotalFeePercent()) /
            feeContract.getFeeDenominator();
        return fee;
    }
    ...
    function calculateFundingFee(
        ...
        int256 newFundingFee = (int256(position.size) *
            (fundingFeeTokenState.accFeePerSize -
                position.accFundingFeePerSize)) / getFundingFeeScalingFactor();

        return
            position.fundingFee +
            (position.isLong ? newFundingFee : -newFundingFee);
    }

    function _addInitialTokenTotalSizes(
    ...
    function _changeMaxTokenTotalSizes(
        TokenTotalSize[] storage tokenTotalSizes
    ) external {
    ...
    function updateTraderOpenPositionId(
        bool isLong,
        uint256 positionId,
    ...
    function clearTraderOpenPositionId(
        bool isLong,
        TraderOpenPositionId storage traderOpenPositionId
    ) internal {
        if (isLong) {
    ...
    function cleanUpPosition(
        ...
        if (position.openPositionIndex != openPositionIds.length - 1) {
            uint256 shiftingPositionId = openPositionIds[
                openPositionIds.length - 1
            ];
            PerpDexLib.Position storage shiftingPosition = positions[
                shiftingPositionId
            ];
        ...
        if (
            positionStatus == PerpDexLib.PositionStatus.Closed ||
            positionStatus == PerpDexLib.PositionStatus.RolledBack
        ) {
    ...
    function cleanUpLimitOrder(
        ...
        require(
            limitOrderPositionToDelete.limitOrderIndex < limitOrderIds.length,
            "Invalid limit order index"
        ); // Should not happen

        if (
            limitOrderPositionToDelete.limitOrderIndex !=
            limitOrderIds.length - 1
        ) {
            uint256 shiftingLimitOrderId = limitOrderIds[
                limitOrderIds.length - 1
            ];
            PerpDexLib.Position storage shiftingLimitOrderPosition = positions[
                shiftingLimitOrderId
            ];
            shiftingLimitOrderPosition
                .limitOrderIndex = limitOrderPositionToDelete.limitOrderIndex;
            limitOrderIds[
                limitOrderPositionToDelete.limitOrderIndex
            ] = shiftingLimitOrderId;
        }
    ...
    function checkPositionSizeAndIncrease(
        TokenTotalSize[] storage tokenTotalSizes,
        TokenType tokenType,
        bool isLong,
        uint256 size
    ...
    function decreaseTotalPositionSize(
        TokenTotalSize[] storage tokenTotalSizes,
        TokenType tokenType,
        bool isLong,
        uint256 size
    ...
    function mergePosition(
        ...
        require(
            (newPosition.statusTime.openTime == block.timestamp &&
                newPosition.positionStatus == PositionStatus.Open) ||
        ...
        require(
            oldPosition.positionStatus == PositionStatus.Open,
            "Old position status is not Open"
        );
        require(currentPrice > 0, "Price is 0");
        require(
            newPosition.traderAddr == oldPosition.traderAddr,
            "Trader is different"
        );
        require(
            newPosition.tokenType == oldPosition.tokenType,
            "TokenType is different"
        );
        require(
            newPosition.isLong == oldPosition.isLong,
            "IsLong is different"
        );
        require(
        ...
        int256 fundingFee = calculateFundingFee(
            oldPosition,
            fundingFeeTokenState
        );
        (int256 pnl, uint256 closeFee) = calculatePnlAndCloseFee(
            oldPosition,
            currentPrice,
            fundingFee,
            c.feeContract
    ...
    function createLimitOrder(
        uint256 positionId,
        PerpDexLib.OpenLimitOrderData memory o,
        uint256 limitOrderIdLength,
        IFee feeContract
    ) internal view returns (Position memory) {
        require(o.wantedPrice > 0, "Price is 0");
        require(
            o.leverage >= 3 && o.leverage <= 100,
    ...
    function _closePosition(
        ...
        FundingFeeTokenState storage fundingFeeTokenState,
        FundingFeeGlobalState storage fundingFeeGlobalState,
        mapping(address => mapping(uint16 => TraderOpenPositionId))
            storage traderOpenPositionIds,
        TokenTotalSize[] storage tokenTotalSizes,
        uint256[] storage openPositionIds,
        uint256 closingPrice,
        ExternalContracts memory c
        ...
        bool isLiquidated = checkAndLiquidatePosition(
            position,
            fundingFeeGlobalState,
            fundingFee,
            pnl,
        ...
        if (isLiquidated) {
            ...
            if (pnl > 0) {
                payAndRecordFee(position, closeFee, false, c.feeContract);
                PerpDexPricesLib.safeTransferAndCheckBalance(
                    position.traderAddr,
                    uint256(marginAfterFundingFee) - closeFee,
                    c.usdt
                );
                ...
                PerpDexPricesLib.safeTransferAndCheckBalance(
                    position.traderAddr,
                    uint256(marginAfterFundingFee) - closeFee - loss,
                    c.usdt
                );
        ...
        cleanUpPosition(
            ...
            traderOpenPositionIds[position.traderAddr][
                uint16(position.tokenType)
            ]
        );
    ...
    function _changeMargin(
        ...
        if (!addMargin) {
            ...
            require(
                pnl > 0
                    ? int256(position.margin - marginDelta) -
                        fundingFee -
                        int256(closeFee) >
                        0
                    : int256(position.margin - marginDelta) -
        ...
        uint256 fee = (marginDelta * c.feeContract.getMarginFeePercent()) /
            c.feeContract.getFeeDenominator();
        require(fee > 0, "Margin added is too small");

        position.margin = addMargin
            ? position.margin + marginDelta - fee
            : position.margin - marginDelta;
        ...
        require(
            0 <= leverage && leverage <= 100 * factor,
            "Leverage is out of boundary"
        );

        if (addMargin) {
            PerpDexPricesLib.safeTransferFromAndCheckBalance(
                position.traderAddr,
                address(this),
                marginDelta,
                c.usdt
        ...
        emit PerpDex.PositionMarginChanged(
            position.positionId,
            position.traderAddr,
            position.tokenType,
            addMargin,
    ...
    function updateFundingFeeState(
        ...
        int256 fundingRate = (long == 0 && short == 0)
            ? int256(0)
            : ((SCALING_FACTOR / COEFFICIENT_DECIMAL_FACTOR) *
                COEFFICIENT *
                (long - short)) / (long + short);
        int256 timeDelta = (fundingFeeState.lastUpdatedTime == 0)
            ? int256(0)
            : int256(block.timestamp - fundingFeeState.lastUpdatedTime);

        int256 timeWeightedFundingRate = (fundingRate * timeDelta) /
            COEFFICIENT_TIME_FACTOR;
        ...
        fundingFeeGlobalState.protocolClaimable += uint256(
            timeWeightedFundingRate * (long - short)
        );
    ...
    function _openPosition(
        ...
        mapping(PerpDexLib.TokenType => PerpDexLib.FundingFeeTokenState)
            storage fundingFeeTokenStates,
        mapping(address => mapping(uint16 => PerpDexLib.TraderOpenPositionId))
            storage traderOpenPositionIds,
        uint256[] storage openPositionIds,
        ...
        uint256 oldPosForMerge = findPositionToMerge(
            newPosition.isLong,
            traderOpenPositionIds[newPosition.traderAddr][
                uint16(newPosition.tokenType)
            ]
        );
    ...
    function _openLimitOrder(
        ...
        if (positions[positionId].positionStatus != PositionStatus.Initial)
            revert NextPositionIdExists(); // Should not happen

        Position memory newPosition = createLimitOrder(
            positionId,
            o,
            limitOrderIds.length,
            c.feeContract
        );
...
library PerpDexBotLib {
    function openPositionForLimitOrder(
        ...
        require(
            position.positionStatus == PerpDexLib.PositionStatus.LimitOrderOpen,
        ...
        PerpDexLib.updateTraderOpenPositionId(
            position.isLong,
            position.positionId,
    ...
    function _executeLimitOrder(
        ...
        PerpDexLib.updateFundingFeeState(
            ...
            fundingFeeGlobalState,
            tokenTotalSizes[uint256(order.tokenType)]
        );

        if (
            !PerpDexLib.checkPositionSizeAndIncrease(
                tokenTotalSizes,
                order.tokenType,
                order.isLong,
                order.size
        ...
        uint256 oldPosForMerge = PerpDexLib.findPositionToMerge(
            order.isLong,
        ...
        if (oldPosForMerge == 0) {
            openPositionForLimitOrder(
                ...
                price,
                openPositionIds,
                traderOpenPositionIds[order.traderAddr][
                    uint16(order.tokenType)
                ],
    ...
    function decreaseSizeAndCleanUpOnLiquidate(
        ...
        PerpDexLib.Position storage position = positions[positionId];
        PerpDexLib.decreaseTotalPositionSize(
            tokenTotalSizes,
            position.tokenType,
            position.isLong,
            position.size
    ...
    function _liquidatePosition(
        ...
        int256 fundingFee = PerpDexLib.calculateFundingFee(
            position,
            fundingFeeTokenStates[position.tokenType]
        );
        (int256 pnl, uint256 closeFee) = PerpDexLib.calculatePnlAndCloseFee(
            position,
            price,
            fundingFee,
            c.feeContract
        ...
        if (
            !PerpDexLib.checkAndLiquidatePosition(
                position,
                fundingFeeGlobalState,
                fundingFee,
                pnl,
    ...
    function _tpslClosePosition(
        ...
        bool shouldClose = position.isLong
            ? (position.slPrice > 0 && price <= position.slPrice) ||
                (position.tpPrice > 0 && price >= position.tpPrice)
            : (position.slPrice > 0 && price >= position.slPrice) ||
                (position.tpPrice > 0 && price <= position.tpPrice);

        if (
            position.statusTime.openTime < updatedAt &&
            position.tpslUpdatedTime < updatedAt &&
            shouldClose
        ) {
    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(PerpDexLib.TokenType => PerpDexLib.FundingFeeTokenState)
        public fundingFeeTokenStates;

    PerpDexLib.TokenTotalSize[] public tokenTotalSizes;

    event SubmittedRoundId(
    ...
    modifier onlyLiquidationAdmin() {
        if (
            !isAdmin(
                adminsByRole[PerpDexAuthLib.AdminType.Liquidation],
                _msgSender()
            )
        ) revert InvalidAdmin();
        _;
    ...
    modifier onlyLimitOrderAdmin() {
        if (
            !isAdmin(
                adminsByRole[PerpDexAuthLib.AdminType.LimitOrder],
                _msgSender()
            )
        ) revert InvalidAdmin();
        _;
    ...
    modifier onlySingleOpenAdmin() {
        if (
            !isAdmin(
                adminsByRole[PerpDexAuthLib.AdminType.SingleOpen],
                _msgSender()
            )
        ) revert InvalidAdmin();
        _;
    ...
    modifier onlyCloseAdmin() {
        if (
            !isAdmin(adminsByRole[PerpDexAuthLib.AdminType.Close], _msgSender())
        ) revert InvalidAdmin();
        _;
    }

    modifier onlyTpslAdmin() {
        if (!isAdmin(adminsByRole[PerpDexAuthLib.AdminType.Tpsl], _msgSender()))
            revert InvalidAdmin();
        _;
    }
    ...
    function setAdmins(
        PerpDexAuthLib.AdminType adminType,
    ...
    function setOracles(
        address _bisonAIRouter,
        address _bisonAISubmission,
        address _pyth
    ) external onlyOwner {
    ...
    function setupAddr(
        ...
        externalContracts = PerpDexLib.ExternalContracts({
            feeContract: IFee(_fee),
            lp: ILP(_lp),
            usdt: IERC20(_usdt)
        });
        require(
            externalContracts.usdt.approve(
    ...
    function getPosition(
        uint256 positionId
    ) external view returns (PerpDexLib.Position memory) {
    ...
    function getPositionIdsForTrader(
        address trader
    ...
    function getTraderOpenPositionIds(
        address trader
    ) external view returns (PerpDexLib.TraderOpenPositionId[] memory) {
        PerpDexLib.TraderOpenPositionId[]
            memory traderOpenPositionIdArr = new PerpDexLib.TraderOpenPositionId[](
                uint256(type(PerpDexLib.TokenType).max) + 1
            );
    ...
    function checkUserSignedData(
        string memory message,
        bytes calldata userSignedData,
        address user
    ) internal {
        PerpDexAuthLib.checkUser(
            message,
    ...
    function openLimitOrder(
        ...
        checkUserSignedData(
            PerpDexAuthLib.getOpenLimitOrderMsg(
                o,
                traderNonce[o.trader],
                address(this)
            ),
            o.userSignedData,
            o.trader
    ...
    function closeLimitOrder(
        uint256 positionId,
        bytes calldata userSignedData
    ) external onlyCloseAdmin nonReentrant {
        PerpDexLib.Position storage order = positions[positionId];
        checkUserSignedData(
            PerpDexAuthLib.getCloseLimitOrderMsg(
                positionId,
                traderNonce[order.traderAddr],
                address(this)
            ),
            userSignedData,
            order.traderAddr
    ...
    function executeLimitOrders(
        uint256[] calldata ordersToExecute,
        uint64[] calldata roundIds,
        PerpDexLib.OraclePrices calldata priceData
    ) external payable onlyLimitOrderAdmin whenNotPaused nonReentrant {
        ...
        ) = PerpDexPricesLib.getPreviousPriceAndTime(
                roundIds,
                priceData,
                bisonAIRouter,
                pyth
        ...
        for (uint256 i = 0; i < ordersToExecute.length; i++) {
            ...
            if (
                order.positionStatus != PerpDexLib.PositionStatus.LimitOrderOpen
            ) {
                // Should not happen
                continue;
            }

            if (prices.length <= uint256(order.tokenType))
                revert InvalidPriceData();
            uint256 price = prices[uint256(order.tokenType)];
            uint256 updatedAt = updatedAts[uint256(order.tokenType)];

            if (
                order.statusTime.limitOpenTime < updatedAt &&
                PerpDexLib.checkExecutionForLimitOrder(order, price)
            ) {
    ...
    function openPosition(
        PerpDexLib.OpenPositionData calldata o
    ) external payable onlySingleOpenAdmin whenNotPaused nonReentrant {
        checkUserSignedData(
            PerpDexAuthLib.getOpenPositionMsg(
                o,
                traderNonce[o.trader],
                address(this)
            ),
        ...
        uint256 positionId = nextPositionId++;
        if (
            positions[positionId].positionStatus !=
            PerpDexLib.PositionStatus.Initial
        ) revert NextPositionIdExists(); // Should not happen

        uint256 currentPrice = PerpDexPricesLib.submitAndGetLatestPrice(
            o.priceData,
            o.tokenType,
        ...
        PerpDexLib.updateFundingFeeState(
            o.tokenType,
            fundingFeeTokenStates[o.tokenType],
            fundingFeeGlobalState,
            tokenTotalSizes[uint256(o.tokenType)]
        );
    ...
    function tpslClosePositions(
        uint256[] calldata positionsToClose,
        uint64[] calldata roundIds,
        PerpDexLib.OraclePrices calldata priceData
    ) external payable onlyTpslAdmin whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < positionsToClose.length; i++) {
            PerpDexLib.Position storage position = positions[
                positionsToClose[i]
            ];
            if (position.positionStatus != PerpDexLib.PositionStatus.Open) {
                // Should not happen
            ...
            if (prices.length <= uint256(position.tokenType))
                revert InvalidPriceData();
    ...
    function closePosition(
        ...
        PerpDexLib.OraclePrices calldata priceData,
        bytes calldata userSignedData
    ) external payable onlyCloseAdmin whenNotPaused nonReentrant {
        PerpDexLib.Position storage position = positions[positionId];

        checkUserSignedData(
            PerpDexAuthLib.getClosePositionMsg(
                positionId,
                traderNonce[position.traderAddr],
                address(this)
            ),
            userSignedData,
            position.traderAddr
    ...
    function liquidatePositions(
        ...
        for (uint256 i = 0; i < liquidatablePositions.length; i++) {
            ...
            if (prices.length <= uint256(position.tokenType))
                revert InvalidPriceData();
            uint256 price = prices[uint256(position.tokenType)];
            uint256 safeUpdatedAt = updatedAts[uint256(position.tokenType)] -
                (
                    priceData.oracleType == PerpDexLib.OracleType.BisonAI
                        ? 10
                        : 0
            ...
            if (
                position.statusTime.openTime < safeUpdatedAt &&
                position.marginUpdatedTime < safeUpdatedAt
            ) {
    ...
    function setTpslPrice(
        uint256 id,
        uint256 tpPrice,
        uint256 slPrice,
        bytes calldata userSignedData
    ) external onlyTpslAdmin whenNotPaused nonReentrant {
        PerpDexLib.Position storage position = positions[id];
        checkUserSignedData(
            PerpDexAuthLib.getSetTpslMsg(
                id,
                tpPrice,
                slPrice,
                traderNonce[position.traderAddr],
    ...
    function submitAndGetBisonAIRoundId(
        PerpDexLib.OraclePrices calldata priceData
    ) external onlyLimitOrLiquidationAdmin nonReentrant {
        PerpDexPricesLib.submitAndGetBisonAIRoundId(
            priceData,
            bisonAISubmissionProxy,
            bisonAIRouter
        );
    ...
    function changeMargin(
        ...
        PerpDexLib.Position storage position = positions[positionId];
        checkUserSignedData(
            PerpDexAuthLib.getChangeMarginMsg(
                positionId,
                addMargin,
                marginDelta,
                traderNonce[position.traderAddr],
    ...
    function claimProtocolFundingFee() external nonReentrant {
        ...
        if (fundingFeeGlobalState.protocolClaimable <= 0)
            revert InvalidProtocolClaimable();
        if (
            fundingFeeGlobalState.protocolClaimable >
            externalContracts.usdt.balanceOf(address(this))
        ) revert InvalidProtocolClaimable();
        PerpDexPricesLib.safeTransferAndCheckBalance(
            admin,
            fundingFeeGlobalState.protocolClaimable,
    ...
    function depositFundingFeeGlobalStateBalance(
        uint256 balance
